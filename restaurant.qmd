...
The following map overlays restaurant data on Maryland block group
boundaries, with restaurants color-coded by their ratings.
...
area in the later section 3.1.2. Ratings are distributed across
locations. It's hard to find a clear trend of higher ratings clustering
in specific regions.
...
distribution is right-skewed, in the range from 0 to 0.5 mostly, with
only a few outliers larger than 0.5.
...
We use a heatmap to present a geographic visualization of average Asian
density by block group. Following the pattern revealed in the previous
graph, we used the scale from 0 to 0.5. The map also shows that areas
with higher Asian density tend to be located in commuting zones, like
the rating distrubution.
...
Both the restaurant ratings and Asian density show a concentration in
the commuting area. Therefore, we narrow down our vision to this
specific region for this section.
...
restaurants in the commuting area results from the narrowing down
process.
...
We also present it with the mapview package for a more interactive
purpose. You may find the rating for each restaurants.
...
The Asian density by block group based on the narrowed-down dataset
clearly shows that higher Asian density is concentrated in the middle
part.
...
let's try a non-spatial regression model first: it may simply be that
the spatial autocorrelation in rating of restaurants is entirely
explained by the Asian density in Maryland.
...
We can see from the simple OLS regression that higher Asian population
density is positively associated with restaurant ratings (p = 0.0164).
For every 1-unit increase in the percentage of Asian residents in an
area, the restaurant rating is associated with about 0.397 points
increase on average.

But median household income and the number of public transport commuters
...
OLS model's fit and residual distribution. The residuals are evenly distributed around a zero line with no clear
...
autocorrelation with effective tools.

------------------------------------------------------------------------

### 4.2 Spacial Residual (Moran's 𝐼)

Moran's 𝐼 is used to test if our error terms are independent of the
spatial distribution. In other words, we are checking whether the residuals we just plotted are randomly distributed across space or show some spatial pattern.
...
For restaurant ratings, Moran's 𝐼 = 0.0605 indicates
a positive spatial autocorrelation. Restaurants with similar ratings tend to be geographically clustered, although the effect is not very strong. And the observed clustering is unlikely due to random chance (p = 0.0071).

However, for OLS regression residuals, Moran's 𝐼 =
0.0338 shows weaker spatial autocorrelation (p = 0.0809). 

Our OLS model has accounted for most of the spatial patterns in the data.
...
The scatterplot again indicates a positive spatial autocorrelation of our restaurant ratings.

------------------------------------------------------------------------

### 4.3 Spacial Regression (SAR)
The spacial autocorrelation is significant and the residual
autorrelation is not significant - our OLS model has already captured the most.

But we still try to include an additional spatial
regression, **spatial autoregressive model (SAR)**, to test if there's any spacial patterns unexplored. The purpose of SAR is to model and account for spatial dependencies or spatial relationships in the data that OLS cannot address. It helps to answer if restaurant ratings in one area are influenced by ratings in neighboring areas.
...
The SAR shows that Asian population density indeed
significantly impacts restaurant ratings. 

However, the weak significance of spatial lag coefficient (Rho) suggests that spatial dependence is present (0.1133) but not strong (p = 0.092).
...
ratings of the existing restaurant. We might have observed sufficient demand in
the market for restaurants, supporting Hotelling's idea that
...
populations) can support multiple businesses. 
...
Besides, it can be analyzed how this
...
affects Chinese restaurant ratings. But no significant spillover effect is observed.
